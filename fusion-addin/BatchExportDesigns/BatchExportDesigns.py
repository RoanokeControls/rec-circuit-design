...
def match_rec_library(deviceset, value, package, rec_devicesets):
    ...
    if value and package:
        ...
        # Strip common single-letter prefixes (C0805 -> 0805, R0402 -> 0402)
        for prefix in ("C", "R", "L"):
...
def generate_master_json(output_dir):
    ...
    Reads *_schematic.json and *_board.json files from output_dir,
    cross-references parts against the REC Standard Library, and
    writes a single master.json.

    Returns the path to master.json, or None if no exports found.
    """
    # ── Locate REC library ──────────────────────────────────────────
    # Try: env var → dev-tree relative path → well-known absolute path
    addin_dir = os.path.dirname(os.path.abspath(__file__))
    candidates = [
        os.environ.get("REC_LIBRARY_PATH", ""),
        os.path.normpath(os.path.join(
            addin_dir, "..", "..", "..",
            "autodesk-coder", "library-data", "rec-library.json"
        )),
        os.path.expanduser(
    ...
    for design_key in all_designs:
        ...
        if brd_data:
            ...
            # Signal layers are numbered 1–16
    ...
    def notify(self, args):
        try:
            ...
            try:
                ...
                pass  # Non-fatal — individual exports still succeeded
...
def run(context):
    ...
    try:
        ...
        # Add to toolbar — FilePanel (Common) is always visible
        panel_ids = [
            "FilePanel",                 # Common tab — always visible
            "UtilityPanel",              # Generic Utilities
            "SolidScriptsAddinsPanel",   # Design workspace Add-Ins
        ]
        added_to = None
        for panel_id in panel_ids:
            ...
            if panel:
                try:
                    existing_ctrl = panel.controls.itemById(BUTTON_ID)
                    if existing_ctrl:
        ...
        if _ui:
            _ui.messageBox(
                "BatchExportDesigns failed to start:\n\n{}".format(
...
def stop(context):
    ...
    try:
        ...
        for panel_id in ["FilePanel", "UtilityPanel", "SolidScriptsAddinsPanel"]:
            panel = ui.allToolbarPanels.itemById(panel_id)
            if panel:
                ctrl = panel.controls.itemById(BUTTON_ID)
                if ctrl:
                    ctrl.deleteMe()
                    break
